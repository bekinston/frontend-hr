...
export const ExperienceList = ({ jobs }) => {
    console.log(jobs);
                ...
                { jobs.map((job, index) => {
                    return (
                        <div style={{marginTop:15, display:'flex', alignItems:'center'}}>
                            ...
                            <NavLink to={`/vacancies/${job.id}/tests`}><button onClick={()=>{window.location.pathname(`/vacancies/${job.id}/tests`)}} style={{marginLeft:70, height:40, boxShadow: '0px 4px 8px rgba(0, 0, 0, 0.15)'}} className={'button-filled-white'}>delete</button></NavLink>
                        </div>
                    )
                })}
        </div>
